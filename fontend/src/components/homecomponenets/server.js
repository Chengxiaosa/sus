...
export default class Server extends Component {
    ...
    getServer = () => {
        axios.get(pjsonUrl+'/server').then(
            ...
            response => {
                this.setState({
                    bandwidth: response.data.server.bandwidth,
                    diskRate: response.data.server.diskRate,
                    request: response.data.server.request,
                    cpu:response.data.server.cpu,
                    memory:response.data.server.memory,
                });
                },
            )
    ...
    componentDidMount() {
        this.timerID= setInterval(this.getServer,1800);
    }
