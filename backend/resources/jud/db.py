...
class DB:
    ...
    def get_mysql(self, config=config['mysql']):
        if not hasattr(self, '_mysql') or self._mysql == None or self._mysql._closed:
            self._mysql = pymysql.connect(**config)     # 单下划线表示受保护变量

        return self._mysql

    def get_mongo(self, config=config['mongo']):
        if not hasattr(self, '_mongo') or self._mongo == None:
            mongo_config = {k: urllib.parse.quote(v) if isinstance(
                v, str) else v for k, v in config.items()}

            uri = 'mongodb://{user}:{password}@{host}:{port}/{database}'.format(
                **mongo_config)
    ...
    def batchpop(self, key, batch_size):
        db = self.get_redis()
        lst = [x for x in [db.lpop(key)
                           for i in range(batch_size)] if x is not None]
        return lst

    def range(self, key, start, end):
        db = self.get_redis()
        lst = db.lrange(key, start, end)
        return lst
